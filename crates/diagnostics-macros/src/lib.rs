...
use syn::{
    Attribute, Data, DeriveInput, Expr, ExprArray, Fields, GenericArgument, Generics, Ident, Index,
    LitStr, Member, Path, PathArguments, Type, parenthesized, parse_macro_input,
};
...
pub fn derive_diagnostic(input: TokenStream) -> TokenStream {
    ...
    expand_diagnostic(&input).unwrap_or_else(syn::Error::into_compile_error).into()
}
...
fn type_argument<'a>(ty: &'a Type, wrapper: &str) -> Option<&'a Type> {
...
fn members_equal(left: &Member, right: &Member) -> bool {
    match (left, right) {
...
fn resolve_runtime_path(explicit: Option<Path>) -> syn::Result<TokenStream2> {
...
fn expand_diagnostic(input: &DeriveInput) -> syn::Result<TokenStream2> {
